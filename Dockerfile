# ============================================
# VERTEX CODERS - PEOPLECORE HTB DOCKERFILE
# ============================================
FROM mcr.microsoft.com/windows/servercore:ltsc2022

# Configurar Shell inicial
SHELL ["powershell", "-Command", "$ErrorActionPreference = 'Stop'; $ProgressPreference = 'SilentlyContinue';"]

# 1. INSTALAR PYTHON 3.11 (Ruta Absoluta C:\Python311)
...
# 2. INSTALAR VC REDIST (Evita WinError 126 en Torch)
...
# 4. INSTALAR OPENSSH (FIX ERROR 0x2 - División en 3 RUNs)
...
# 5. DEPENDENCIAS DE PYTHON (Uso de ruta absoluta para Pip)
...
# 6. PRE-DESCARGA DEL MODELO IA (Para funcionamiento Offline)
RUN C:\Python311\python.exe -c "from transformers import AutoTokenizer, AutoModelForCausalLM; \
    AutoTokenizer.from_pretrained('Qwen/Qwen2.5-0.5B-Instruct'); \
    AutoModelForCausalLM.from_pretrained('Qwen/Qwen2.5-0.5B-Instruct')"

# 7. CÓDIGO Y CONFIGURACIÓN DE RED TEAMING
COPY . .

# Ejecutar setup.ps1 para SMB y WinRM
RUN powershell.exe -ExecutionPolicy Bypass -File ./setup.ps1

# 8. CONFIGURACIÓN DE USUARIO Y FLAGS (Estándar HTB)
...
# 10. COMANDO DE ARRANQUE (Servicios + Waitress)
